...
/* PE INSTRUCTIONS */
static const uint16_t pic32_pe_loader []=// <editor-fold defaultstate="collapsed" desc="PIC32MM loader">
{
    // [LOW] [HIGH]
    0x41A3, 0xFF20,
    0x41A5, 0xDEAD,
...
static void PIC32MM_SetMode(uint8_t numbits, uint8_t value) // <editor-fold defaultstate="collapsed" desc="Transmits the "numbits" LSbs of "value" as TMS bits on JTAG 2W 4PH.  TDI = 0, TDO is ignored">
{
    uint8_t i;
...
static uint8_t PIC32MM_DataIO(uint8_t numbits, uint8_t tdi, uint8_t tms) // <editor-fold defaultstate="collapsed" desc="Transmits up to 8 bits at a time on TDI/TMS and receives up to 8 bits from TDO as the return value.">
{
    uint8_t i;
...
static void PIC32MM_SendCommand(uint8_t command) // <editor-fold defaultstate="collapsed" desc="Transmits a 5 bit command via JTAG 2W 4PH">
{
    //TMS header 1100
    PIC32MM_SetMode(4, 0x03); // sent LSb first
    // command itself
    PIC32MM_DataIO(5, command, 0x10); // TMS on MSB
    // TMS footer 10
    PIC32MM_SetMode(2, 0x01); // sent LSb first
} // </editor-fold>

static uint8_t PIC32MM_XferData8(uint8_t byte0) // <editor-fold defaultstate="collapsed" desc="Completes an 8-bit XferData psuedo operation">
{
    uint8_t rxbyte=0;

    //TMS header 100
    PIC32MM_SetMode(3, 0x01); // sent LSb first
    // TDO first bit received on last phase of setmode
    if(asm_temp3&0x80)
        rxbyte=1;
    // data
    PIC32MM_DataIO(8, byte0, 0x80); // TMS on last bit
    rxbyte|=(asm_temp3<<1);
    // TMS footer 10
    PIC32MM_SetMode(2, 0x01); // sent LSb first  
...
static uint8_t PIC32MM_XferData32(uint32_t iData, uint32_t *oData, char rxdata) // <editor-fold defaultstate="collapsed" desc="Completes a 32-bit XferData psuedo op.  If rxdata > 0, places received data in the Upload buffer.">
{
    uint32_t data=0;
    uint32_t rxbyte=0;
    uint8_t thirdbyte=0;
    ...
    PIC32MM_DataIO(8, byte3, 0x80); // TMS on last bit
    rxbyte|=(asm_temp3<<1);
...
static void PIC32MM_XferFastData32(uint32_t iData) // <editor-fold defaultstate="collapsed" desc="Completes a 32-bit XferFastData psuedo op.  If PrAcc = 0 sets Pk2Status.ICDTimeout">
{
    uint8_t byte3=(uint8_t) (iData>>24);
    uint8_t byte2=(uint8_t) (iData>>16);
    uint8_t byte1=(uint8_t) (iData>>8);
...
static void PIC32MM_XferInstruction(uint32_t instruction) // <editor-fold defaultstate="collapsed" desc="Completes the entire XferInstruction psuedo op. Has a timeout on PrACC bit check, which sets Pk2Status.ICDTimeout">
{
    elapse_t elapse;
    uint8_t praccbyte;

    PIC32MM_SendCommand(ETAP_CONTROL);
    Elapse_Create(elapse, 1400);

    do
    {
        praccbyte=PIC32MM_XferData32(0x0004D000, NULL, 0);
    ...
    PIC32MM_SendCommand(ETAP_DATA);
    // actual instruction:
    PIC32MM_XferData32(instruction, NULL, 0);
    PIC32MM_SendCommand(ETAP_CONTROL);
    PIC32MM_XferData32(0x0000C000, NULL, 0);
} // </editor-fold>

bool PIC32MM_CheckDeviceStatus(void) // <editor-fold defaultstate="collapsed" desc="Get device status">
{
    elapse_t elapse;
    mchp_stt_t statusVal;

    PIC32MM_SetMode(6, 0x1F);
    PIC32MM_SendCommand(MTAP_SW_MTAP);
    PIC32MM_SendCommand(MTAP_COMMAND);

    Elapse_Create(elapse, 1400);

    do
    {
        statusVal.Value=PIC32MM_XferData8(MCHP_STATUS);
...
bool PIC32MM_MTAP_ChipErase(void) // <editor-fold defaultstate="collapsed" desc="Erase memory of device">
{
    elapse_t elapse;
    mchp_stt_t statusVal;
    ...
    PIC32MM_SendCommand(MTAP_SW_MTAP);
    PIC32MM_SendCommand(MTAP_COMMAND);
    PIC32MM_XferData8(MCHP_ERASE);
    PIC32MM_XferData8(MCHP_DE_ASSERT_RST);
    Elapse_Create(elapse, 1400);
    ...
    while(1)
    {
        ...
        if(Elapse_Get(elapse))
        {
            icsp_log(ICSP_LOG_ERASE_ERROR, NULL, NULL, NULL);
            return 0;
        }
    }
} // </editor-fold>

uint32_t PIC32MM_MTAP_GetDeviceID(void) // <editor-fold defaultstate="collapsed" desc="Get device ID">
{
    uint32_t Id;

    PIC32MM_SendCommand(MTAP_IDCODE);
    PIC32MM_XferData32(32, &Id, 1);

    return Id;
} // </editor-fold>

static void PIC32MM_EnterSerialExecution(void) // <editor-fold defaultstate="collapsed" desc="Enter serial excecution">
{
    elapse_t elapse;
    uint8_t statusVal;
    ...
    if(serial_execution_mode==0)
    {
        ...
        do
        {
            statusVal=PIC32MM_XferData8(MCHP_STATUS);
        ...
        PIC32MM_SendCommand(MTAP_SW_MTAP);
        PIC32MM_SendCommand(MTAP_COMMAND);
        PIC32MM_XferData8(MCHP_ASSERT_RST);
        PIC32MM_SendCommand(MTAP_SW_ETAP);
        PIC32MM_SetMode(6, 0x1F);
        PIC32MM_SendCommand(ETAP_EJTAGBOOT);
        PIC32MM_SendCommand(MTAP_SW_MTAP);
        PIC32MM_SendCommand(MTAP_COMMAND);
        PIC32MM_XferData8(MCHP_DE_ASSERT_RST);
...
bool PIC32MM_DownloadPE(void) // <editor-fold defaultstate="collapsed" desc="Download MCHP PE">
{
    int i;
    uint32_t nwords=PIC32_PE_INSTRUCTION_LEN;

    PIC32MM_EnterSerialExecution();
    /* Step 1. */
    PIC32MM_SendCommand(MTAP_SW_ETAP);
    PIC32MM_SetMode(6, 0x1F);
    PIC32MM_XferInstruction(0xA00041A4);
    PIC32MM_XferInstruction(0x02005084);
    ...
    for(i=0; i<PIC32_PE_LOADER_LEN; i+=2)
    {
        ...
        PIC32MM_XferInstruction(opcode1); // lui a2, PE_loader_hi++
        PIC32MM_XferInstruction(opcode2); // ori a2, PE_loader_lo++
        PIC32MM_XferInstruction(0x6e42eb40); // sw  a2, 0(a0)
    }
    /* Step 3. Jump to PE loader @0xA000.0201. */
    PIC32MM_XferInstruction(0xa00041b9);
    PIC32MM_XferInstruction(0x02015339);
    PIC32MM_XferInstruction(0x0c004599);
    PIC32MM_XferInstruction(0x0c000c00);
    PIC32MM_XferInstruction(0x0c000c00);
    /* Switch from serial to fast execution mode. */
    PIC32MM_SendCommand(MTAP_SW_ETAP);
    PIC32MM_SetMode(6, 0x1F);
    PIC32MM_SendCommand(ETAP_FASTDATA);
    PIC32MM_XferFastData32(0xa0000300);
    PIC32MM_XferFastData32(nwords);
    /* Download the PE itself (step 7-B). */
    for(i=0; i<nwords; i++)
        PIC32MM_XferFastData32(pic32_pe_instruction[i]);

    icsp_delay_ms(100);
    /* Download the PE instructions. */
    PIC32MM_XferFastData32(0x00000000); /* Step 8 - jump to PE. */
    PIC32MM_XferFastData32(0xDEAD0000);
    icsp_delay_ms(100);

    if((0xFFFF&PIC32MM_PE_ExecVerison())==PIC32_PE_VERSION_MM)
        return 1;

    return 0;
} // </editor-fold>

static uint32_t PIC32MM_GetPEResponse(uint8_t savedata, uint8_t execute) // <editor-fold defaultstate="collapsed" desc="Completes the entire GET PE RESPONSE psuedo op. Has a timeout on PrACC bit check, which sets Pk2Status.ICDTimeout">
{
    elapse_t elapse;
    uint8_t praccbyte;
    uint32_t response=0;
    ...
    PIC32MM_SendCommand(ETAP_DATA);
    // actual instruction:
    PIC32MM_XferData32(0, &response, savedata); // response in upload buffer
...
static bool PIC32MM_SendPECmdGetResp(uint32_t Opcode, uint32_t Operand, const uint32_t *pTxD,
                                     uint32_t TxLen, uint32_t *pRxD, uint32_t RxLen) // <editor-fold defaultstate="collapsed" desc="PE Send command then get Response">
{
    ...
    PIC32MM_SendCommand(ETAP_FASTDATA);
    PIC32MM_XferFastData32((Opcode<<16)|Operand);

    for(i=0; i<TxLen; i++)
        PIC32MM_XferFastData32(*pTxD++);
    ...
    while(1) // Get 1st response
    {
        ...
        resp.Value=PIC32MM_GetPEResponse(1, 1);
    ...
    while(1) // Get other responses
    {
        ...
        *pRxD=PIC32MM_GetPEResponse(1, 1);
...
bool PIC32MM_PE_RowProgram(uint32_t startAddress, const uint32_t *pData, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Programs one row of Flash memory at the specified address">
{
    bool response;
    uint32_t *pbuff;
    ...
    response=PIC32MM_SendPECmdGetResp(PE_ROW_PROGRAM, PE_CMD_NO_OPERAND, &pbuff[0], length+1, NULL, 0);
    free(pbuff);

    if(response)
        return 1;

    icsp_log(ICSP_LOG_ROW_PROG_ERROR, &startAddress, NULL, NULL);

    return 0;
} // </editor-fold>

bool PIC32MM_PE_Read(uint32_t startAddress, uint32_t *pData, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Reads N 32-bit words of memory starting from the specified address (N < 65,536)">
{
    bool response;
    ...
    response=PIC32MM_SendPECmdGetResp(PE_READ, length, &startAddress, 1, &pData[0], length);

    if(response)
        return 1;

    icsp_log(ICSP_LOG_READ_ERROR, &startAddress, NULL, NULL);

    return 0;
} // </editor-fold>

bool PIC32MM_PE_Program(uint32_t startAddress, const uint32_t *pData, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Programs Flash memory starting at the specified address">
{
    bool response;
    uint32_t *pbuff;
    ...
    response=PIC32MM_SendPECmdGetResp(PE_PROGRAM, PE_CMD_NO_OPERAND, &pbuff[0], length+2, NULL, 0);
    free(pbuff);

    return response;
} // </editor-fold>

bool PIC32MM_PE_ChipErase(uint32_t startAddress) // <editor-fold defaultstate="collapsed" desc="Chip Erase of entire chip">
{
    if(!icsp_valid_address_check(startAddress, 1))
        return 0;

    return PIC32MM_SendPECmdGetResp(PE_CHIP_ERASE, PE_CMD_NO_OPERAND, &startAddress, 1, NULL, 0);
} // </editor-fold>

bool PIC32MM_PE_PageErase(uint32_t startAddress, uint32_t nPage) // <editor-fold defaultstate="collapsed" desc="Erases pages of code memory from the specified address">
{
    if(!icsp_valid_address_check(startAddress, nPage*TARGET_PAGE_WORD_SIZE))
        return 0;

    return PIC32MM_SendPECmdGetResp(PE_PAGE_ERASE, nPage, &startAddress, 1, NULL, 0);
} // </editor-fold>

bool PIC32MM_PE_BlankCheck(uint32_t startAddress, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Blank Check code">
{
    bool response;
    ...
    response=PIC32MM_SendPECmdGetResp(PE_BLANK_CHECK, PE_CMD_NO_OPERAND, &buff[0], 2, NULL, 0);

    if(response)
        return 1;

    icsp_log(ICSP_LOG_NOT_BLANK_ERROR, &startAddress, NULL, NULL);

    return 0;
} // </editor-fold>

uint16_t PIC32MM_PE_ExecVerison(void) // <editor-fold defaultstate="collapsed" desc="Reads the PE software version">
{
    uint32_t version;

    PIC32MM_SendPECmdGetResp(PE_EXEC_VERSION, PE_CMD_NO_OPERAND, NULL, 0, &version, 1);
    // ICSP_Debug("PE ver: %04X\n", version&0xFFFF);

    return (uint16_t) version;
} // </editor-fold>

uint32_t PIC32MM_PE_GetCRC(uint32_t startAddress, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Gets the CRC of Flash memory">
{
    uint32_t buff[2]={startAddress, length};
    ...
    PIC32MM_SendPECmdGetResp(PE_GET_CRC, PE_CMD_NO_OPERAND, &buff[0], 2, &buff[0], 1);

    return buff[0];
} // </editor-fold>

bool PIC32MM_PE_ProgramCluster(uint32_t startAddress, uint32_t length) // <editor-fold defaultstate="collapsed" desc="Programs the specified number of bytes to the specified address">
{
    uint32_t buff[2]={startAddress, length};
    ...
    return PIC32MM_SendPECmdGetResp(PE_PROGRAM_CLUSTER, PE_CMD_NO_OPERAND, &buff[0], 2, NULL, 0);
} // </editor-fold>

uint16_t PIC32MM_PE_GetDeviceID(void) // <editor-fold defaultstate="collapsed" desc="Returns the hardware ID of the device">
{
    uint32_t chipid;

    PIC32MM_SendPECmdGetResp(PE_GET_DEVICEID, PE_CMD_NO_OPERAND, NULL, 0, &chipid, 1);

    return (uint16_t) chipid;
} // </editor-fold>

bool PIC32MM_PE_GetCheckSum(uint32_t startAddress, uint32_t length, uint8_t Cks) // <editor-fold defaultstate="collapsed" desc="Gets the checksum of Flash memory">
{
    uint32_t buff[2]={startAddress, length};
    ...
    PIC32MM_SendPECmdGetResp(PE_GET_CHECKSUM, PE_CMD_NO_OPERAND, &buff[0], 2, &buff[0], 1);

    if((uint8_t) buff[0]==Cks)
        return 1;

    icsp_log(ICSP_LOG_CHECKSUM_ERROR, &startAddress, &buff[0], NULL);

    return 0;
} // </editor-fold>

bool PIC32MM_PE_DoubleWordProg(uint32_t Address, uint32_t Word1, uint32_t Word0) // <editor-fold defaultstate="collapsed" desc="Programs two words of Flash memory at the specified address">
{
    bool response;
    ...
    {
        icsp_log(ICSP_LOG_ADDR_NOT_ALIG_DWORD, &Address, NULL, NULL);
        return 0;
    }

    response=PIC32MM_SendPECmdGetResp(PE_DOUBLE_WORD_PGRM, PE_CMD_NO_OPERAND, buff, 3, NULL, 0);

    if(response)
        return 1;

    icsp_log(ICSP_LOG_DWORD_PROG_ERROR, &Address, NULL, NULL);

    return 0;
} // </editor-fold>

void PIC32MM_UI_GetUDID(uint32_t *pUDID) // <editor-fold defaultstate="collapsed" desc="Get UDID of target chip">
{
#if defined(TARGET_UDID_START) && defined(TARGET_UDID_END)
    PIC32MM_PE_Read(UDID_START, pUDID, 5);
#endif
} // </editor-fold>

void PIC32MM_InitICSP(void) // <editor-fold defaultstate="collapsed" desc="Initializes the GPIOs required for ICSP">
{
...
static int8_t PIC32MM_InvalidID(uint32_t ID) // <editor-fold defaultstate="collapsed" desc="Check ID">
{
    static const uint32_t IdList[TARGET_ID_COUNT]={TARGET_ID_LIST};

    int8_t i;

    ID&=0x0FFFFFFF;

    for(i=0; i<TARGET_ID_COUNT; i++)
    {
        if(ID==IdList[i])
            return i;
    }
...
static uint8_t *PIC32MM_GetChipName(uint8_t IdIdx) // <editor-fold defaultstate="collapsed" desc="Get chip name>
{
    static const uint8_t IdName[TARGET_ID_COUNT][18]={TARGET_ID_NAME};

    if(IdIdx<TARGET_ID_COUNT)
        return (uint8_t*) &IdName[IdIdx][0];

    return NULL;
} // </editor-fold>

bool PIC32MM_EnterICSP(void) // <editor-fold defaultstate="collapsed" desc="ICSP entry">
{
    if(Icsp_Is_InProgress==0)
    {
        ...
        if(PIC32MM_CheckDeviceStatus())
        {
            int8_t IdIdx;

            DevID=PIC32MM_MTAP_GetDeviceID();
            // ICSP_Debug("Target ID: %08X\n", DevID);
            IdIdx=PIC32MM_InvalidID(DevID);

            if(IdIdx>0) //(DevID&0x0FFFFFFF)==TARGET_ID)
            {
                icsp_log(ICSP_LOG_SUCCESS, &DevID, NULL, PIC32MM_GetChipName(IdIdx));
        ...
        PIC32MM_ExitICSP();
        icsp_log(ICSP_LOG_CHIPID_ERROR, &DevID, NULL, NULL);
...
void PIC32MM_ExitICSP(void) // <editor-fold defaultstate="collapsed" desc="Exit ICSP mode">
{
    PIC32MM_SetMode(6, 0x1F);
    ICSP_PGD_SetDigitalInput();
    wait();
...
static uint32_t PIC32MM_Convert4x8to32(const uint8_t *pArr) // <editor-fold defaultstate="collapsed" desc="Convert 32bit to 4x8bit">
{
...
bool PIC32MM_NVM_WriteDWord_Then_Checksum(icsp_nvm_t *pNvm) // <editor-fold defaultstate="collapsed" desc="Write double word then checksum">
{
    bool rslt=0;
...
int8_t PIC32MM_NVM_Write(uint32_t Addr, const uint8_t *pData, int Len) // <editor-fold defaultstate="collapsed" desc="Write hex data to flash">
{
#define debug(...) printf(__VA_ARGS__)
    ...
    if(Len==0)
    {
        if(!PIC32MM_NVM_WriteDWord_Then_Checksum(&nvm))
        {
            rslt=PROC_ERR;
    ...
    while(Len>0)
    {
        ...
        {
            nvm.Word0=PIC32MM_Convert4x8to32(pData);
        }
        else // if((Addr&4)==4) // Word 1 address
        {
            nvm.Word1=PIC32MM_Convert4x8to32(pData);
        }
