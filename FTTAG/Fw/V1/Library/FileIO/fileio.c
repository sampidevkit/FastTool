...
void FILEIO_DrivePropertiesGet(FILEIO_DRIVE_PROPERTIES * properties, char driveId)
{
    ...
    if(properties->new_request==true)
    {
        ...
        /* Settings base_locationd on FAT type */
        switch(drive->type)
        {
            case FILEIO_FILE_SYSTEM_TYPE_FAT32:
                properties->prv.EndClusterLimit=FILEIO_CLUSTER_VALUE_FAT32_END;
                properties->prv.ClusterFailValue=FILEIO_CLUSTER_VALUE_FAT32_FAIL;
                break;
            case FILEIO_FILE_SYSTEM_TYPE_FAT16:
                properties->prv.EndClusterLimit=FILEIO_CLUSTER_VALUE_FAT16_END;
                properties->prv.ClusterFailValue=FILEIO_CLUSTER_VALUE_FAT16_FAIL;
                break;
            case FILEIO_FILE_SYSTEM_TYPE_FAT12:
                properties->prv.EndClusterLimit=FILEIO_CLUSTER_VALUE_FAT12_END;
                properties->prv.ClusterFailValue=FILEIO_CLUSTER_VALUE_FAT16_FAIL;
                break;
        }
        ...
        properties->prv.curcls=properties->prv.c;
        FILEIO_FATRead(drive, properties->prv.c);
    }
    ...
    {
        // look at its value
        if((value=FILEIO_FATRead(drive, properties->prv.c))==properties->prv.ClusterFailValue)
        {
        ...
        properties->prv.c++; // check next cluster in FAT
        // check if reached last cluster in FAT, re-start from top
        if((value==properties->prv.EndClusterLimit)||(properties->prv.c>=(properties->results.total_clusters+2)))
            properties->prv.c=2;

        // check if full circle done, disk full
        if(properties->prv.c==properties->prv.curcls)
        {
