...
{ // fabricate an MBR structure in the RAM buffer segment
    ...
    { // segment 6: 0x180 - 0x1bf
        ...
        buffer[0x3E]=0x00; // Status - 0x80 (bootable), 0x00 (not bootable), other (error)
        buffer[0x3F]=0x01; // Cylinder
    }
    else
    { // segment 7: 0x1c0 - 0x1ff
...
const uint8_t VBR_seg0[]={
    ...
    /* 0x00b */ (uint8_t) FILEIO_CONFIG_MEDIA_SECTOR_SIZE, // Bytes per sector
    /* 0x00c */ (uint8_t) (FILEIO_CONFIG_MEDIA_SECTOR_SIZE>>8),
...
{ // fabricate an MBR structure in the RAM buffer
    ...
    if(0==seg) // segment from 000 to 0x03f
    {
        ...
        memcpy((void*) &buffer[0x2B], (void*) FILEIO_DISK_NAME, 11);
        // FAT system ( 8 bytes)
        memcpy((void*) &buffer[0x36], (void*) "FAT12   ", 8);
    }
    else if(seg==7) // patch in the signature
    {
        buffer[0x3E]=0x55;
        buffer[0x3F]=0xAA; // signature End of sector (0x55AA)
    }
    // segments 1-6 from 0x040 to 0x1c0 are empty
}
...
void FATRecordGet(uint8_t * buffer, uint8_t seg)
{
    ...
    if(seg==0)
    {
        ...
        buffer[1]=0xFF;
        buffer[2]=0xFF;
        buffer[3]=0xFF; // cluster 2 - first/last cluster in short file chain
        buffer[4]=0x0F; // INFO.TXT
    }
}
...
const uint8_t entry0[ROOT_ENTRY_SIZE]={
    ...
    FILEIO_DISK_NAME_IN_BYTE,
    0x08, // Specify this entry as a volume label
    0x00, // Reserved

    0x00, // Creation time, fine res 10 ms units (0-199)
    (uint8_t)TIMEL(FILES_MAJOR, FILES_MINOR, 0), // Creation time, hour/min/sec
    (uint8_t)TIMEH(FILES_MAJOR, FILES_MINOR, 0), // Creation time, hour/min/sec
    (uint8_t)DATEL(FILES_YEAR, FILES_MONTH, FILES_DAY), // Creation date, YMD
    (uint8_t)DATEH(FILES_YEAR, FILES_MONTH, FILES_DAY), // Creation date, YMD

    (uint8_t)DATEL(FILES_YEAR, FILES_MONTH, FILES_DAY), // Last Access date, YMD
    (uint8_t)DATEH(FILES_YEAR, FILES_MONTH, FILES_DAY), // Last Access date, YMD
    0x00, 0x00, // Extended Attributes

    (uint8_t)TIMEL(FILES_MAJOR, FILES_MINOR, 0), // Last Modified time h/m/s
    (uint8_t)TIMEH(FILES_MAJOR, FILES_MINOR, 0), // Last Modified time h/m/s
    (uint8_t)DATEL(FILES_YEAR, FILES_MONTH, FILES_DAY), // Last Modified date, YMD
    (uint8_t)DATEH(FILES_YEAR, FILES_MONTH, FILES_DAY), // Last Modified date, YMD
...
const uint8_t entry1[ROOT_ENTRY_SIZE]={
    'I', 'N', 'F', 'O', ' ', ' ', ' ', ' ', // File name (exactly 8 characters)
    ...
    0x00,
    0x00, 0x00, 0x00, // INFO string size
};
...
void RootRecordGet(uint8_t * buffer, uint8_t seg)
{
    ...
    if(seg==0)
    {
        ...
        // add the INFO.TXT file
        memcpy((void*) &buffer[ROOT_ENTRY_SIZE], (const void*) entry1, ROOT_ENTRY_SIZE);
        // length of INFO.TXT file, max 65535 bytes
        buffer[60]=(uint8_t) FileIO_InfoSize();
        buffer[61]=(uint8_t) (FileIO_InfoSize()>>8);
    }
}
