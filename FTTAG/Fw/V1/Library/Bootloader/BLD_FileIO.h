...
uint16_t BLD_FileIo_InfoSize(void);
void BLD_FileIo_InfoGet(uint8_t *pD, uint8_t seg);
uint8_t BLD_FileIo_MediaDetect(void* config);
FILEIO_MEDIA_INFORMATION * BLD_FileIo_MediaInitialize(void* config);
uint8_t BLD_FileIo_SectorRead(void* config, uint32_t sector_addr, uint8_t* buffer, uint8_t seg);
uint8_t BLD_FileIo_SectorWrite(void* config, uint32_t sector_addr, uint8_t* buffer, uint8_t seg);
uint16_t BLD_FileIo_SectorSizeRead(void* config);
uint32_t BLD_FileIo_CapacityRead(void* config);
uint8_t BLD_FileIo_WriteProtectStateGet(void* config);
void BLD_FileIo_Initialize(void);
