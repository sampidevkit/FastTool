...
public bool VCP_Debug_IsTxReady(void) // <editor-fold defaultstate="collapsed" desc="UART is TX ready">
{
    return ((VcpDbgTxBuf.ready==1)&&(VcpDbgTxBuf.len<CDC_DATA_IN_EP_SIZE));
} // </editor-fold>

public bool VCP_Debug_IsTxDone(void) // <editor-fold defaultstate="collapsed" desc="UART is TX done">
{
    if(VcpDbgTxBuf.ready)
        return (VcpDbgTxBuf.len==0);
    ...
    if(VcpDbgTxBuf.ready)
    {
        ...
        if(VcpDbgRxBuf.len>0)
        {
            ...
            if(VcpDbgRxBuf.cbfnc!=NULL)
                VcpDbgRxBuf.cbfnc(VcpDbgRxBuf.buf, VcpDbgRxBuf.len);
        }
        // USB CDC0 put data
        switch(VcpDbgTxBuf.next)
        {
            case 0:
                if(Tick_Timer_Is_Over_Ms(VcpDbgTxBuf.tk, 1)||(VcpDbgTxBuf.len==CDC_DATA_IN_EP_SIZE))
                {
...
public void VCP_Debug_SetRxCallback(void (*cbfnc)(const uint8_t *, uint8_t)) // <editor-fold defaultstate="collapsed" desc="VCP Debug set RX callback">
{
