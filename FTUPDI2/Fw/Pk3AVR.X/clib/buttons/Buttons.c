...
bt_stt_t BUTTON_GetState(bt_cxt_t *pBtCxt, bool preInput) // <editor-fold defaultstate="collapsed" desc="Button UP">
{
    switch(pBtCxt->DoNext)
    {
        ...
        case SINGLE_PRESS:
            if(preInput==1)
            {
                if(Tick_Dif(Tick_Get(), (tick_t)pBtCxt->Begin)>=(TICK_PER_MS*SINGLE_PRESS_INTERVAL))
                {
            ...
            else if(Tick_Dif(Tick_Get(), (tick_t)pBtCxt->Begin)>=(TICK_PER_MS*HOLD_PRESS_INTERVAL))
            {
        ...
        case NEXT_PRESS:
            ...
            else if(Tick_Dif(Tick_Get(), (tick_t)pBtCxt->Begin)>=(TICK_PER_MS*BUTTON_RELEASE_INTERVAL))
            {
                pBtCxt->DoNext=NOT_PRESS;
